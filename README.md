...
- 保存原始捕获为 PCAP，支持同一 Ethernet、Loopback 或 Raw IP 链路中的 IPv4/IPv6 混合流量；导出摘要为 UTF-8 CSV。
- 捕获数量、队列丢弃、解析异常和成功重组等状态统计。
- 可配置时间窗的实时包速率/字节速率折线图；固定一秒采样，空闲区间补 0，停止后立即归零。
- TCP/UDP 双向会话统计，以及按序列号整理的 Follow TCP Stream。
- 深度解析 DNS 查询、HTTP 首部、TLS 握手/SNI/ALPN 和 DHCP 选项；TLS ClientHello 支持有界 TCP 分段重组，无法确认的载荷仍明确标为端口推断。
- 顶栏设置和帮助窗口已实现；列表缓存、界面刷新间隔和流量图时间窗可配置并持久化。
...
较早记录已被淘汰。Ethernet、Loopback 与 Raw IP 是不同链路层，需要分别保存；但同一链路层内的
IPv4 和 IPv6 可以保存到一个 PCAP。Raw IPv4/IPv6 使用标准 DLT_RAW，回环流量使用 DLT_NULL。
...
超时分片、TLS ClientHello 跨 TCP 分段、队列上限、网卡筛选、IPv4/IPv6 混合 PCAP 保存与回读、
PCAP/CSV 和离屏 GUI 初始化。
...
- IPv6 Fragment Header 已解析，但当前只对 IPv4 分片做完整重组；TCP 重组仅用于有界 TLS ClientHello 元数据解析和会话观察，不做完整 TCP 状态机或 TLS 解密。
