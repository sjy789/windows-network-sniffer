...
|---|---|---|---|
| `sjy789 / shijiayu` | 总体架构、基础协议解析、IPv4 分片重组、NetScope GUI 与系统集成 | 介绍系统目标、架构和主界面，演示实时抓包与 IPv4 分片 | 提供总体架构图、GUI 截图和系统集成说明 |
| `Narcissismm / YangHaoWen` | 流量态势、会话/TCP 流、应用层元数据、异常检测与跨模块缺陷修复 | 演示流量曲线、会话分析、明文/加密载荷差异和异常告警 | 提供高级分析章节、缺陷修复说明和测试结果 |
| `hopeworld0218 / mengxi zhang` | IPv6、扩展首部链、ICMPv6、邻居发现和 IPv6 过滤 | 对照 Wireshark 演示 IPv6/ICMPv6 字段与扩展首部 | **PPT 撰写与统稿**：统一视觉、架构图、演示截图和汇报节奏 |
| `coisini612 / coisini-612` | 离线 PCAP/PCAPNG、后台加载、取消与进度、离线链路类型 | 演示打开 PCAP、后台加载、取消以及离线数据进入统一分析流水线 | **项目设计报告撰写与统稿**：整合需求、设计、算法、测试、结论与参考资料 |
...
PPT 制作者（`hopeworld0218 / mengxi zhang`）负责控制页面结构与总时长；报告统稿者（`coisini612 / coisini-612`）负责确保 PPT、README、设计报告
